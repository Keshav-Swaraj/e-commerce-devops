...
export const getFeaturedProducts = async (req, res) => {
	try {
		...
		try {
			...
			console.warn("[Redis] Cache read failed, fetching from DB:", redisError.message);
		}
		...
		// store in redis for future quick access (best effort)
		try {
...
export const deleteProduct = async (req, res) => {
	try {
		...
		if (product.image) {
			// Extract the public_id including the folder (e.g., "products/abc123")
			const urlParts = product.image.split("/");
			const fileWithExt = urlParts[urlParts.length - 1];
			const fileName = fileWithExt.split(".")[0];
			const folderIndex = urlParts.indexOf("upload");
			const publicId = folderIndex !== -1
				? urlParts.slice(folderIndex + 2).join("/").replace(/\.[^/.]+$/, "")
				: `products/${fileName}`;
			try {
